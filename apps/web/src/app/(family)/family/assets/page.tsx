...
export default function FamilyAssetsPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      fetchAssets()
      fetchSummary()
      toast.success("Bem familiar registado com sucesso")
  ...
  const handleDelete = (id: string) => {
    toast("Eliminar este bem?", {
      ...
      action: {
        label: "Eliminar",
        onClick: async () => {
          await assetsApi.remove(id, ctx()).catch(() => {})
          setCursor(null)
          setHasMore(false)
          fetchAssets()
          fetchSummary()
  ...
  const contextHeaders = getContextHeader()

  return (
    <div>
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-6">
        <h2 className="text-2xl font-bold tracking-tight">Património Familiar</h2>
        <Dialog open={createOpen} onOpenChange={(v) => { setCreateOpen(v); if (!v) resetForm() }}>
          <DialogTrigger render={<Button className="hidden md:inline-flex" />}>
            <Plus className="h-4 w-4 mr-1" /> Novo bem
          </DialogTrigger>
          <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
            <DialogHeader><DialogTitle>Registar bem familiar</DialogTitle></DialogHeader>
