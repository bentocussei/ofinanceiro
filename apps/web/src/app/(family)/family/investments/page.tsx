...
export default function FamilyInvestmentsPage() {
  ...
  const ctx = (): { headers: Record<string, string> } => ({ headers: getContextHeader() })
  ...
  const fetchAllocation = () => {
    investmentsApi.allocation(ctx()).then(setAllocation).catch(() => {})
  }

  const fetchInsights = () => {
    investmentsApi.insights(ctx()).then(setInsightsData).catch(() => {})
  }
  ...
  const handleAsk = async () => {
    ...
    setAskAnswer("")
    try {
      const res = await investmentsApi.ask(q, ctx())
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ----------------------------------------------------------------- */}
      <div>
        <h3 className="font-semibold mb-3">Investimentos da família</h3>
      ...
      <div className="rounded-xl bg-card p-5 shadow-sm border border-border">
        ...
        <div className="flex gap-2">
          <Input
            placeholder="Ex: Devo diversificar mais o portfólio da família?"
            value={askQuestion}
