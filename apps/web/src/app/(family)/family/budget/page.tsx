...
export default function FamilyBudgetPage() {
  ...
  const ctxHeaders = getContextHeader()
  const opts = { headers: ctxHeaders }

  const fetchBudgets = () => {
    budgetsApi.list(undefined, opts).then((data) => {
      setBudgets(data ?? [])
      ;(data ?? []).forEach((b) => {
        budgetsApi.status(b.id, opts)
