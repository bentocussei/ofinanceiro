...
import { reportsApi, type CategorySpending, type ReportSummary, type MemberSpending } from "@/lib/api/reports"
...
export default function FamilyReportsPage() {
  ...
  const fetchData = (p: Period) => {
    ...
    reportsApi.spendingByCategory(dateFrom, dateTo, ctx)
      .then(setSpending).catch(() => {})

    reportsApi.incomeExpenseSummary(dateFrom, dateTo, ctx)
      .then(setSummary).catch(() => {})

    reportsApi.monthlySummary(p, ctx)
      .then((d) => setMembers((d.by_member ?? []) as MemberSpending[]))
      .catch(() => {})
  }
  ...
  return (
    <div>
        ...
        <div className="grid gap-4 md:grid-cols-3 mb-8">
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {summary.income_count ?? 0} transacções
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {summary.expense_count ?? 0} transacções
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
      ...
      {members.length > 0 && (
        <div className="rounded-lg border bg-card p-4 mb-8">
          <h3 className="text-sm font-semibold mb-4">Gastos por Membro</h3>
            ...
            {members.map((m) => (
              <div key={m.member_name} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-sm font-mono text-green-500">+{formatKz(m.income ?? 0)}</span>
                  <span className="text-sm font-mono text-red-500">-{formatKz(m.expense ?? 0)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Spending by Category Pie Chart + Details */}
      {spending.length > 0 ? (
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <div className="space-y-3">
              {spending.map((cat, i) => {
                ...
                return (
                  <div key={cat.category_id || cat.category_name} className="flex items-center gap-3">
                    <div
        ...
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mt-3">
            Registe transacções familiares para ver os relatórios
          </p>
        </div>
      )}
    </div>
